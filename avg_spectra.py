...
def stats(source, num, seed, device=torch.device('cuda' if torch.cuda.is_available() else "cpu")):
...
def calc(source, dest, mean, std, num, seed, beta, interp, device=torch.device('cuda' if torch.cuda.is_available() else "cpu")):
